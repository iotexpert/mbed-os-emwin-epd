                       ...
                       cy_eink_update_t updateType, bool powerCycle)
{
    CY_ASSERT(prevFrame);
    CY_ASSERT(newFrame);
    CY_ASSERT(updateType == CY_EINK_FULL_4STAGE || updateType == CY_EINK_FULL_2STAGE || updateType == CY_EINK_PARTIAL );
    ...
    switch(updateType)
    {
        case CY_EINK_PARTIAL:
            /* If partial update, update the display with changes from previous frame */
            Pv_EINK_PartialStageHandler(prevFrame, newFrame);
            break;

        case CY_EINK_FULL_2STAGE:
            /* For full 2 stage update:
                1. Update the display with inverse of previous frame
                2. Update the display with new frame */
        ...
        case CY_EINK_FULL_4STAGE:
            /* For full 4 stage:
                1. Update display with inverse of previous frame
                2. Update display with white frame
                3. Update display with inverse of new frame
                4. Update display with new frame */
            Pv_EINK_FullStageHandler(prevFrame, PV_EINK_STAGE1);
